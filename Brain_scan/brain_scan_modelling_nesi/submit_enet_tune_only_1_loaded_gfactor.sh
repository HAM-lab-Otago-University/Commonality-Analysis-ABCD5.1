...
for DATASET in smri_T2_mean_total_data.RDS smri_T1_mean_total_data.RDS


do      
  DATANAME=$(basename $DATASET .RDS)
  JOBID=$(sbatch -J enet_tune_loaded_gfactor.${DATANAME} --array=1-${N_BATCHES} enet_tune_loaded_gfactor_rsmri.sh ${DATASET} )
