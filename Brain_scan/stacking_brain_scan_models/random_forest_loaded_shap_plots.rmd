...
output_list <- readRDS(paste0(nesi_folder,"stacking_gfactor_modelling/rf_shap_results/",file_name))
return(output_list)
}
...
# SRC_SUBJECT_ID that is subject ID
...
shapley_data_process_plotting <- function(shapley_input, data_input){
  ...
  shapley_large_wider <- shapley_input%>%tibble::as_tibble()%>%
    ...
    mutate(SRC_SUBJECT_ID=data_input$SRC_SUBJECT_ID)
 
  
  shapley_large<-  shapley_large_wider%>%
    pivot_longer(-SRC_SUBJECT_ID, names_to = "modality_names", values_to = "shapley_values") %>%
  ...
  data_large <- data_input%>%tibble::as_tibble()%>%
    dplyr::select(ends_with("large"),"SRC_SUBJECT_ID")%>%
    pivot_longer(-SRC_SUBJECT_ID, names_to = "modality_names", values_to = "feature_values")%>%
    mutate(modality_names = str_remove_all(.data[["modality_names"]],"large"))
  ...
  data_small <- data_input%>%tibble::as_tibble()%>%
    dplyr::select(ends_with("small"),"SRC_SUBJECT_ID")%>%
    pivot_longer(-SRC_SUBJECT_ID, names_to = "modality_names", values_to = "feature_values")%>%
    mutate(modality_names = str_remove_all(.data[["modality_names"]],"small"))
  ...
  shapley_summary_tibble <- plyr::join_all(list(shapley_all,data_all), 
                               by=c("SRC_SUBJECT_ID","modality_names"),type="full")%>%
                              unique.data.frame()
