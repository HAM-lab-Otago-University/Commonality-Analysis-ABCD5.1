...
extract_rf_data <- function(modality_input){
 list_input <-  readRDS(paste0("/Volumes/sci-psy-narun/Nesi/Yue/","stacking_gfactor_modelling/collect_enet_results_loaded_gfactor/",modality_input,".enet_results.RDS"))
...
random_forest_baseline_train <- map(.x = site_char,~random_forest_data_cross_site(site_input = .))

random_forest_baseline_test <- map(.x = site_char,~random_forest_data_cross_site(list_input =rf_baseline_test,site_input = .))

random_forest_followup_train <- map(.x = site_char,~random_forest_data_cross_site(list_input =rf_followup_train,site_input = .))

random_forest_followup_test <- map(.x = site_char,~random_forest_data_cross_site(list_input =rf_followup_test,site_input = .))


new_names <- str_remove_all(scan_names,"data")
new_names <- str_remove_all(new_names,"ROI\\_")

test_rf_site <- map(random_forest_baseline_test,~select(.x,SITE_ID_L)%>% unique())%>% do.call(rbind,.)
train_rf_site <- map(random_forest_baseline_train,~select(.x,SITE_ID_L)%>% unique())
followup_test_rf_site <- map(random_forest_followup_test,~select(.x,SITE_ID_L)%>% unique())%>% do.call(rbind,.)

identical(test_rf_site,followup_test_rf_site)

names(random_forest_baseline_train) <- test_rf_site$SITE_ID_L

names(random_forest_baseline_test) <- test_rf_site$SITE_ID_L

names(random_forest_followup_train) <- test_rf_site$SITE_ID_L

names(random_forest_followup_test) <- test_rf_site$SITE_ID_L
...
saveRDS(random_forest_baseline_test, paste0(scriptfold,"stacking_gfactor_modelling/random_forest_data/random_forest_all_baseline_test_5.1.RDS"))

saveRDS(random_forest_followup_test, paste0(scriptfold,"stacking_gfactor_modelling/random_forest_data/random_forest_all_followup_test_5.1.RDS"))
```
...
enet_pred_values <- function(site_input = site_char[1],
                             data_input = random_forest_baseline){
  train_input <- data_input[["train_list"]][[site_input]]%>%
  ...
  test_input <- data_input[["test_list"]][[site_input]]%>%
                 select(all_of(subj_info),ends_with("_large"))%>% 
                 naniar::replace_with_na_all(condition = ~.x == 1000)%>%
                 mutate(fold = "test")
...
enet_pred_baseline <- map(site_char,~enet_pred_values(site_input = .))

enet_pred_followup <- map(site_char,~enet_pred_values(site_input = .,
...
map2(.x = enet_pred_baseline, .y = site_char,
     ~write_csv(.x, paste0(scriptfold,"stacking_gfactor_modelling/random_forest_data/enet_pred_baseline_",.y,".csv"))
)


map2(.x = enet_pred_followup, .y = site_char,
     ~write_csv(.x, paste0(scriptfold,"stacking_gfactor_modelling/random_forest_data/enet_pred_followup_",.y,".csv"))
)
