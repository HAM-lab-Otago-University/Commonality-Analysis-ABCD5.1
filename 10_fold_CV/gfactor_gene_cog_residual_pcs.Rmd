...
```{r}
  General_Cognitive_Function_Davies <- General_Cognitive_Function_Davies[order(General_Cognitive_Function_Davies$SRC_SUBJECT_ID),]

  General_Cognitive_Function_Savage <- General_Cognitive_Function_Savage[order(General_Cognitive_Function_Savage$SRC_SUBJECT_ID),]

Cognitive_Performance_lee <- Cognitive_Performance_lee[order(Cognitive_Performance_lee$SRC_SUBJECT_ID),]
## check whether the ID variables are the same
identical(General_Cognitive_Function_Davies$SRC_SUBJECT_ID,General_Cognitive_Function_Savage$SRC_SUBJECT_ID)

identical(General_Cognitive_Function_Davies$SRC_SUBJECT_ID,Cognitive_Performance_lee$SRC_SUBJECT_ID)
## find out the IDs that are in the genetic data table not in the site data table 
#uncorrected_id <- setdiff(General_Cognitive_Function_Davies$SRC_SUBJECT_ID,Site_baseline$SRC_SUBJECT_ID)
## These are the mismatched IDs if the previous code does not run, run this
uncorrected_id <- c("NDAR_INV3GFFD7GXB","NDAR_INV3PTRFM5","NDAR_INV44RDE6R","NDAR_INV5LMTFDU","NDAR_INV64PZCK82","NDAR_INV67KWNPR7","NDAR_INV6D8NMY0B","NDAR_INV9PUZ8D0B","NDAR_INVANUGPBRY","NDAR_INVK6RANURAB","NDAR_INVK71S0G4Z","NDAR_INVLW9XT5PS","NDAR_INVP39ZM2J","NDAR_INVPF0NSFWX","NDAR_INVTBYKL2UL","NDAR_INVUKK77ZR","NDAR_INVURZU1N9B","NDAR_INVVR39TB4C","NDAR_INVX6KVGNJ","NDAR_INVYUTT0RGD","NDARINVPWLFYWTX")


corrected_id <- c("NDAR_INV3GFD7GXB","NDAR_INV3PTPRFM5","NDAR_INVV44RDE6R","NDAR_INV5LM4TFDU","NDAR_INV6UPZCK82","NDAR_INV67KWVPR7","NDAR_INVGD8NMY0B","NDAR_INV9PUA8D0B","NDAR_INVANUGPBR4","NDAR_INVK6RANRAB","NDAR_INVK7150G4Z","NDAR_INVLW9XT5P3","NDAR_INVP3G9ZM2J","NDAR_INVPF0N5FWX","NDAR_INVTB7KL2UL","NDAR_INVNUKK77ZR","NDAR_INVURZU1N9V","NDAR_INVVR93TB4C","NDAR_INVNX6KVGNJ","NDAR_INVYUTY0RGD","NDAR_INVPWLFYWTX")  

change_ID_tibble <- tibble(wrong_id = uncorrected_id,
                          right_id = corrected_id )

change_ID_tibble_keep <- change_ID_tibble%>%
                         filter(right_id %in% setdiff(corrected_id,General_Cognitive_Function_Davies$SRC_SUBJECT_ID))

change_ID_tibble_remove <- change_ID_tibble%>%
                         filter(!right_id %in% setdiff(corrected_id,General_Cognitive_Function_Davies$SRC_SUBJECT_ID))

print(change_ID_tibble_keep)

print(change_ID_tibble_remove)


General_Cognitive_Function_Davies <- General_Cognitive_Function_Davies %>% 
                                     filter(!SRC_SUBJECT_ID %in% change_ID_tibble_remove$wrong_id)
...
for(i in 1:dim(change_ID_tibble_keep)[1]){
  ...
  General_Cognitive_Function_Davies$SRC_SUBJECT_ID[which(General_Cognitive_Function_Davies$SRC_SUBJECT_ID== wrong_id)] <- right_id
  General_Cognitive_Function_Savage$SRC_SUBJECT_ID[which(General_Cognitive_Function_Savage$SRC_SUBJECT_ID== wrong_id)] <- right_id
  Cognitive_Performance_lee$SRC_SUBJECT_ID[which(Cognitive_Performance_lee$SRC_SUBJECT_ID== wrong_id)] <- right_id
}
### the following 6 subjects do not have information about site in the ABCD 5.1 release but in ABCD 4.0 release
setdiff(General_Cognitive_Function_Davies$SRC_SUBJECT_ID,acspsw03_baseline$SRC_SUBJECT_ID)

### count site with genetics data
left_join(General_Cognitive_Function_Davies,acspsw03_baseline,by = "SRC_SUBJECT_ID")%>%
count(SITE_ID_L)  
```
...
ACSselected %>%
 ...
 skimr::skim()#knitr::kable(glimpse(ACS))
...
ACSselected_baseline_selected <-ACSselected_baseline %>% filter(SRC_SUBJECT_ID %in% change_ID_tibble_keep$right_id)
...
sumCog_select <- left_join(sumCog_select,ACSselected,by =c("SRC_SUBJECT_ID","EVENTNAME"))%>%
  drop_na(SITE_ID_L) %>%
...
sumCog_fold <- left_join(sumCog_select,fold_info, by = c("SRC_SUBJECT_ID","EVENTNAME")) %>% drop_na(fold)

fold_names <- unique(sumCog_fold$fold) %>% set_names()


sumCog_select_split <- map(fold_names, ~split_func(.x,data_input = sumCog_fold)) 
...
genetics_score_extract <- function(score_select_input, tibble_input,score_name_input){
  selected_tibble <- tibble_input %>% select(all_of(c("SRC_SUBJECT_ID",score_select_input)))
  names(selected_tibble) <- c("SRC_SUBJECT_ID",score_name_input)
  return(selected_tibble)
}

### pc scores are the same across all files
genetics_pc_tibble <- General_Cognitive_Function_Davies %>% select(all_of(c("SRC_SUBJECT_ID",paste0("pc", seq(1:4))))) 
...
genetics_data_processing <- function(data_list= genetics_data_list, split_input){
  ### get the subject ID and the site information
   training_subj_site <-  training(split_input)%>% 
                                            filter(EVENTNAME == "baseline_year_1_arm_1")%>%
                                            select(all_of(c("SRC_SUBJECT_ID", "SITE_ID_L")))
...
train_data_list <- data_list %>% map(.,~left_join(.,computed_gfactor_list[["output_train_baseline"]],by = "SRC_SUBJECT_ID"))
test_data_list <- data_list %>% map(.,~left_join(.,computed_gfactor_list[["output_test_baseline"]],by = "SRC_SUBJECT_ID"))
...
train_score_selected <- pmap(list(train_data_list,best_score_names,genetics_score_features),
                                ~genetics_score_extract(score_select_input = ..2,
                                                        tibble_input = ..1,
                                                        score_name_input=..3))
train_score_tibble <- plyr::join_all(train_score_selected, by = "SRC_SUBJECT_ID", type = "full")


output_data_train <- plyr::join_all(list(train_score_tibble,
                                         training_subj_site,
                                         genetics_pc_tibble),by = "SRC_SUBJECT_ID")%>% drop_na()

test_score_selected <- pmap(list(test_data_list,best_score_names,genetics_score_features),
                                ~genetics_score_extract(score_select_input = ..2,
                                                        tibble_input = ..1,
                                                        score_name_input=..3))
test_score_tibble <- plyr::join_all(test_score_selected, by = "SRC_SUBJECT_ID", type = "full")
...
genetics_score_test_data_site <- genetics_score_test_data %>% map(.,~select(.,all_of(c("SRC_SUBJECT_ID" ,"SITE_ID_L"))))%>% 
                          do.call(rbind,.) %>% 
                          filter(! SRC_SUBJECT_ID %in% removed_subj) 
genetics_score_test_data_site%>% nrow()
...
genetics_cor_tibble_all <- map2(.x =genetics_cor_tibble ,.y = fold_names,~mutate(.x, fold = .y ))%>% do.call(rbind,.)
```
...
General_Cognitive_Function_Davies_gfactor <- left_join(General_Cognitive_Function_Davies,
                                                       baseline_test_tibble, by="SRC_SUBJECT_ID")
...
```{r}
genetics_gfactor_baseline_train <-map2(.x=genetics_score_train_data,.y =baseline_train_gfactor,~full_join(.x,.y,by="SRC_SUBJECT_ID")%>%
                                    drop_na())

genetics_gfactor_baseline_test <- map2(.x=genetics_score_test_data,.y =baseline_test_gfactor,~full_join(.x,.y,by="SRC_SUBJECT_ID")%>%
                                    drop_na())

genetics_gfactor_followup_train <- map2(.x=genetics_score_train_data,.y =followup_train_gfactor,~full_join(.x,.y,by="SRC_SUBJECT_ID")%>%
                                    drop_na())

genetics_gfactor_followup_test <- map2(.x=genetics_score_test_data,.y =followup_test_gfactor,~full_join(.x,.y,by="SRC_SUBJECT_ID")%>%
                                    drop_na())

genetics_gfactor_baseline_train_select <- genetics_gfactor_baseline_train %>% map(.,~select(.,-all_of(c("SRC_SUBJECT_ID","SITE_ID_L"))))

genetics_gfactor_baseline_test_select <- genetics_gfactor_baseline_test%>% map(.,~select(.,-all_of(c("SRC_SUBJECT_ID","SITE_ID_L"))))

genetics_gfactor_followup_train_select <- genetics_gfactor_followup_train%>% map(.,~select(.,-all_of(c("SRC_SUBJECT_ID","SITE_ID_L"))))

genetics_gfactor_followup_test_select <- genetics_gfactor_followup_test%>% map(.,~select(.,-all_of(c("SRC_SUBJECT_ID","SITE_ID_L"))))
...
```{r,eval=FALSE}
saveRDS(output_list,paste0(scriptfold,'genetics_psychopathology_common_scan_all_scripts/psychopathology_cog_gene_pred_residual_fold', '.RData'))
...
saveRDS(output_table,paste0(scriptfold,'Common_psy_gene_brain_all/saved_outputs/performance_metrics/gene_performance_metric_fold', '.RData'))
